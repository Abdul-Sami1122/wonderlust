...
async function initializeMapTiler() {
  if (!maptilersdk) {
    try {
      const { config, geocoding } = await import("@maptiler/sdk"); // Use named exports
      maptilersdk = { config, geocoding }; // Store config and geocoding
...
module.exports.createListing = async (req, res, next) => {
  try {
    ...
    const geocodeResult = await maptilersdk.geocoding.forward(query, { limit: 1 });
...
module.exports.trending = async (req, res) => {
  const allListings = await listing.find({ category: "trending" }).sort({ createdAt: -1 });
  res.render("listings/index.ejs", { allListings });
};
// Rooms callback to show rooms data
module.exports.rooms = async (req, res) => {
  const allListings = await listing.find({ category: "rooms" }).sort({ createdAt: -1 });
  res.render("listings/index.ejs", { allListings });
};
// Iconic Cities callback to show rooms data
module.exports.iconic_cities = async (req, res) => {
  const allListings = await listing.find({ category: "iconic_cities" }).sort({ createdAt: -1 });
  res.render("listings/index.ejs", { allListings });
};
// Mountain Hotels callback to show rooms data
module.exports.mountain = async (req, res) => {
  const allListings = await listing.find({ category: "mountain" }).sort({ createdAt: -1 });
  res.render("listings/index.ejs", { allListings });
};
// Castels Hotels callback to show rooms data
module.exports.castels = async (req, res) => {
  const allListings = await listing.find({ category: "castels" }).sort({ createdAt: -1 });
  res.render("listings/index.ejs", { allListings });
};
// Amazing Pools Hotels callback to show rooms data
module.exports.amazing_pools = async (req, res) => {
  const allListings = await listing.find({ category: "amazing_pools" }).sort({ createdAt: -1 });
  res.render("listings/index.ejs", { allListings });
};
// Camping Hotels callback to show rooms data
module.exports.camping = async (req, res) => {
  const allListings = await listing.find({ category: "camping" }).sort({ createdAt: -1 });
  res.render("listings/index.ejs", { allListings });
};
// Farms side hotels callback to show rooms data
module.exports.farms = async (req, res) => {
  const allListings = await listing.find({ category: "farms" }).sort({ createdAt: -1 });
  res.render("listings/index.ejs", { allListings });
};
// Arctic hotels callback to show rooms data
module.exports.arctic = async (req, res) => {
  const allListings = await listing.find({ category: "arctic" }).sort({ createdAt: -1 });
  res.render("listings/index.ejs", { allListings });
};
